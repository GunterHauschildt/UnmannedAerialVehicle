...
class Level_one(tf.keras.Model):
    def __init__(self):
        with tf.name_scope(name="Level_one") as scope:
            ...
            self.conv1b = tf.keras.layers.Conv2D(filters=64, kernel_size=(3, 3), strides=(2, 2), padding="same",
                                                 kernel_regularizer=tf.keras.regularizers.l2(l=0.0))
            self.bn_1b = tf.keras.layers.BatchNormalization()

            self.conv1c = tf.keras.layers.Conv2D(filters=64, kernel_size=(3, 3), strides=(2, 2), padding="same",
                                                 kernel_regularizer=tf.keras.regularizers.l2(l=0.0))
            ...
            self.conv1e = tf.keras.layers.Conv2D(filters=64, kernel_size=(3, 3), strides=(1, 1), padding="same",
                                                 kernel_regularizer=tf.keras.regularizers.l2(l=0.0))
            self.bn_1e = tf.keras.layers.BatchNormalization()

            self.conv1f = tf.keras.layers.Conv2D(filters=64, kernel_size=(3, 3), strides=(1, 1), padding="same",
                                                 kernel_regularizer=tf.keras.regularizers.l2(l=0.0))
            self.bn_1f = tf.keras.layers.BatchNormalization()

            self.conv_output = tf.keras.layers.Conv2D(filters=4, kernel_size=(1, 1), strides=(1, 1), padding="same",
...
class Lucas_Kanade_layer():
    def __init__(self, batch_size, height_template, width_template, num_channels, mask_region_size=0):
        ...
        self.p_W_p_p = self.partial_W_partial_p(self.batch_size, self.height_template, self.width_template,
                                                self.num_channels)
    ...
    def Gradient_F(self, template_feature):
        ...
        template_feature_partial_u = (template_feature[:, 1:height + 1, 2:, :] - template_feature[:, 1:height + 1,
                                                                                 :width, :]) / 1.0
        template_feature_partial_v = (template_feature[:, 2:, 1:width + 1, :] - template_feature[:, :height,
                                                                                1:width + 1, :]) / 1.0
    ...
    def update_matrix(self, template_feature_real, input_feature_real, matrix_real):
        ...
        for i in range(real_bs):
            template_feature = template_feature_real[i,
                               int(self.mask_region_size / 2):(int(self.mask_region_size / 2) + self.height_template),
                               int(self.mask_region_size / 2):(int(self.mask_region_size / 2) + self.width_template), :]
...
class ResNet_first_input(tf.keras.Model):
    def __init__(self, if_regular=False):
        ...
        with tf.name_scope(name="ResNet_first_input") as scope:
            ...
            self.conv1_pre = tf.keras.layers.Conv2D(filters=64, kernel_size=(3, 3), strides=(1, 1), padding="same",
            ...
            self.conv1_aft = tf.keras.layers.Conv2D(filters=64, kernel_size=(3, 3), strides=(1, 1), padding="same",
                                                    kernel_regularizer=tf.keras.regularizers.l2(l=0.05))

            self.bn_1_aft = tf.keras.layers.BatchNormalization()

            if self.if_regular:
                self.conv1_forge = tf.keras.layers.Conv2D(filters=1, kernel_size=(3, 3), strides=(1, 1), padding="same",
                                                          kernel_regularizer=tf.keras.regularizers.l2(l=0.05))
            else:
                self.conv1_forge = tf.keras.layers.Conv2D(filters=64, kernel_size=(3, 3), strides=(1, 1),
                                                          padding="same",
                                                          kernel_regularizer=tf.keras.regularizers.l2(l=0.05))
...
class ResNet_second_input(tf.keras.Model):
    def __init__(self, if_regular=False):
        ...
        with tf.name_scope(name="ResNet_second_input") as scope:
            ...
            self.conv1_pre = tf.keras.layers.Conv2D(filters=64, kernel_size=(3, 3), strides=(2, 2), padding="same",
                                                    kernel_regularizer=tf.keras.regularizers.l2(l=0.05))
...
class Net_first(tf.keras.Model):
    def __init__(self):
        with tf.name_scope(name="Net_first") as scope:
            ...
            self.conv1a = tf.keras.layers.Conv2D(filters=32, kernel_size=(3, 3), strides=(1, 1), padding="same",
                                                 kernel_regularizer=tf.keras.regularizers.l2(l=0.05))

            self.bn_1a = tf.keras.layers.BatchNormalization()

            self.conv1b = tf.keras.layers.Conv2D(filters=32, kernel_size=(3, 3), strides=(1, 1), padding="same",
                                                 kernel_regularizer=tf.keras.regularizers.l2(l=0.05))

            self.bn_1b = tf.keras.layers.BatchNormalization()

            self.conv2a = tf.keras.layers.Conv2D(filters=64, kernel_size=(3, 3), strides=(1, 1), padding="same",
                                                 kernel_regularizer=tf.keras.regularizers.l2(l=0.05))
            ...
            self.conv3a = tf.keras.layers.Conv2D(filters=64, kernel_size=(3, 3), strides=(1, 1), padding="same",
                                                 kernel_regularizer=tf.keras.regularizers.l2(l=0.05))

            self.bn_3a = tf.keras.layers.BatchNormalization()

            self.conv3b = tf.keras.layers.Conv2D(filters=64, kernel_size=(3, 3), strides=(1, 1), padding="same",
                                                 kernel_regularizer=tf.keras.regularizers.l2(l=0.05))

            self.bn_3b = tf.keras.layers.BatchNormalization()

            self.conv4a = tf.keras.layers.Conv2D(filters=128, kernel_size=(3, 3), strides=(1, 1), padding="same",
                                                 kernel_regularizer=tf.keras.regularizers.l2(l=0.05))

            self.bn_4a = tf.keras.layers.BatchNormalization()

            self.conv4b = tf.keras.layers.Conv2D(filters=128, kernel_size=(3, 3), strides=(1, 1), padding="same",
                                                 kernel_regularizer=tf.keras.regularizers.l2(l=0.05))

            self.bn_4b = tf.keras.layers.BatchNormalization()

            self.conv5a = tf.keras.layers.Conv2D(filters=128, kernel_size=(3, 3), strides=(1, 1), padding="same",
                                                 kernel_regularizer=tf.keras.regularizers.l2(l=0.05))

            self.bn_5a = tf.keras.layers.BatchNormalization()

            self.conv5b = tf.keras.layers.Conv2D(filters=128, kernel_size=(3, 3), strides=(1, 1), padding="same",
                                                 kernel_regularizer=tf.keras.regularizers.l2(l=0.05))

            self.bn_5b = tf.keras.layers.BatchNormalization()

            self.conv6a = tf.keras.layers.Conv2D(filters=256, kernel_size=(3, 3), strides=(1, 1), padding="same",
                                                 kernel_regularizer=tf.keras.regularizers.l2(l=0.05))

            self.bn_6a = tf.keras.layers.BatchNormalization()

            self.conv6b = tf.keras.layers.Conv2D(filters=256, kernel_size=(3, 3), strides=(1, 1), padding="same",
                                                 kernel_regularizer=tf.keras.regularizers.l2(l=0.05))
